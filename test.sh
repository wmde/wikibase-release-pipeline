...
docker run --env-file "$ENV_FILE"  -v "$ROOT/git_cache:/srv/git" docker-registry.wikimedia.org/releng/quibble-stretch-php73 --packages-source composer --db sqlite --skip selenium,npm-test,phpunit-standalone,api-testing --project-branch mediawiki/core="$BRANCH" --project-branch mediawiki/extensions/UniversalLanguageSelector="$BRANCH" --project-branch mediawiki/extensions/WikibaseCirrusSearch="$BRANCH" --project-branch mediawiki/extensions/cldr="$BRANCH" mediawiki/extensions/UniversalLanguageSelector mediawiki/extensions/WikibaseCirrusSearch mediawiki/extensions/cldr