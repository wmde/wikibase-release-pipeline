...
To interact with the WDQS frontend, navigate to the URL defined as `WDQS_PUBLIC_HOST` in the `.env` file. By default, this is set to `query.example`.

Alternatively, send `GET` requests with your SPARQL query to the WDQS frontend endpoint:
`https://query.example/sparql?query={SPARQL}`
