...
status
status "🔧 Wikibase Deploy CLI Installer"
status
...
WIKIBASE_PUBLIC_HOST_DEFAULT=${WIKIBASE_PUBLIC_HOST:-$SERVER_IP.nip.io}
...
while true; do
  ...
  if [ -z "$MW_ADMIN_PASS" ]; then
    ...
    status "❌ Password must be at least 10 characters. Try again."
  else
    break
  fi
done
