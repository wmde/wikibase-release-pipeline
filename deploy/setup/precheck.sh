...
check_version() {
    ...
    if [ -n "$version" ] && [[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ...
        RESULTS+="❌ FAIL: Unable to determine $software version or not installed"$'\n'
    fi
}
...
check_domain_resolution() {
    ...
    if [ -n "$ip_address" ]; then
        RESULTS+="✅ PASS: $domain found (resolves to $ip_address)"$'\n'
    else
        RESULTS+="❌ FAIL: $domain not found"$'\n'
    fi
}
...
check_domain_resolution "$WIKIBASE_PUBLIC_HOST"

# Check architecture
if [ "$(uname -m)" == "x86_64" ] || [ "$(uname -m)" == "amd64" ]; then
    RESULTS+="✅ PASS: Computer architecture is AMD64"$'\n'
else
    RESULTS+="❌ FAIL: Computer architecture is not AMD64 (ARM not currently supported)"$'\n'
fi
...
if grep -q "FAIL" <<< "$RESULTS"; then
    echo "Please review above and WBS Deploy documentation to resolve issues."
else
    echo "✅ All ready to run WBS Deploy! Run \`./deploy\` 🚀"
fi
