...
artifactsPath = (
    "https://api.github.com/repos/wmde/wikibase-release-prototype/actions/runs/"
    + os.getenv("WORKFLOW_RUN_NUMBER")
    + "/artifacts"
)
...
if not os.path.exists(basepath):
    for artifact in artifactsMetadata["artifacts"]:
        print(
            "Downloading %d bytes from %s"
            % (artifact["size_in_bytes"], artifact["name"])
        )
