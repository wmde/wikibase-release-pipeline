...
Each managed source must be declared as structured data in the current image's `docker-bake.hcl`. The same manifest supplies those pins to the Docker build, so the repository URL, ref, and revision have one authority and misspelled or incomplete provider entries fail clearly. Buildx evaluates the manifest; the update tooling uses a concrete-syntax-tree parser to replace individual string literals without reformatting HCL or discarding comments. A newly introduced revision may be empty for `wbs-dev update` to populate. When that source did not exist in the previous release, the generated changelog records it as added and links to the selected commit instead of constructing a comparison from a nonexistent prior revision.
