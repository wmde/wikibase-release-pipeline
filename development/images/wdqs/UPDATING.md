...
4. Updates `WDQS.version` in [`docker-bake.hcl`](./docker-bake.hcl), drafts the changelog, asks the operator to confirm the image version, and leaves every change unstaged for review with `git diff`.
...
1. Read the current `WDQS.version` from [`docker-bake.hcl`](./docker-bake.hcl) and inspect newer [`query-service-parent-*` tags](https://github.com/wikimedia/wikidata-query-rdf/tags).
...
`wbs-dev update` does not select the Java or Debian images in [`docker-bake.hcl`](./docker-bake.hcl). When reviewing them:
