...
const CLI = resolve( 'scripts/wbs-dev/cli.ts' );
const TSX = resolve( 'node_modules/.bin/tsx' );
const SOURCE_UPDATER = resolve( 'scripts/update-sources/update_sources.py' );
...
describe( 'wbs-dev release tooling', () => {
	afterEach( () => {
	...
	describe( 'release preparation', () => {
		it( 'updates only the exact requested source variable', () => {
			const script = [
				...
				`spec = importlib.util.spec_from_file_location("updater", ${ JSON.stringify( SOURCE_UPDATER ) })`,
				'module = importlib.util.module_from_spec(spec)',
				'spec.loader.exec_module(module)',
				'print(module.replace_variable("OAUTH_COMMIT=aaa\\nWSOAUTH_COMMIT=bbb\\n", "OAUTH_COMMIT", "ccc"), end="")'
			].join( '; ' );
			assert.equal(
				run( 'python3', [ '-B', '-c', script ], process.cwd() ),
				'OAUTH_COMMIT=ccc\nWSOAUTH_COMMIT=bbb\n'
			);
		} );
