...
import { fetchRemoteTags, git, remoteTagCommit } from './git.js';
import type { ReleaseProject } from './projects.js';
import { assertStableVersion, hasChangelogVersion } from './versioning.js';
...
function readTarget( project: ReleaseProject ): ReleaseTarget {
	...
	return {
		...
		tag,
		existingCommit: remoteTagCommit( tag )
	};
}

export function preflightRelease( projects: ReleaseProject[] ): ReleaseTarget[] {
	if ( git( [ 'status', '--porcelain' ] ) ) {
		throw new Error( 'Release requires a clean working tree.' );
	}
	fetchRemoteTags();
	if ( !git( [ 'branch', '-r', '--contains', 'HEAD' ], { allowFailure: true } ) ) {
		throw new Error( 'Release requires HEAD to exist on a remote branch.' );
	}
	return projects.map( readTarget );
}
