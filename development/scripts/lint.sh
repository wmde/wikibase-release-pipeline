...
# Always run in the wbs-dev container to have the extra dependencies available (currently Python and Hadolint)
...
if [[ ! -f /.dockerenv  ]]; then
  ...
  exec docker compose --progress quiet run --build --rm wbs-dev -c 'scripts/lint.sh "$@"' -- "$@"
fi
